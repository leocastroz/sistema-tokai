...
export async function getMarca() {
  try {
    const response = await supabase.get(`/rest/v1/marca_postorota`);
...
export async function addMarca(newCollaborator) {
  try {
    const response = await supabase.post('/rest/v1/marca_postorota', newCollaborator);
    return response.data;
  } catch (error) {
...
export async function deleteMarca(professorId) {
  try {
    await supabase.delete(`/rest/v1/marca_postorota?id=eq.${professorId}`);
  } catch (error) {
...
export async function editMarca(professorId, data) {   
  try {
    await supabase.put(`/rest/v1/marca_postorota?id=eq.${professorId}`, data);
  } catch (error) {
