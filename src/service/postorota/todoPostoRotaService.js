...
export async function getClient(user_id) {
  try {
    const response = await supabase.get(`/rest/v1/todo_postorota?select=*&user_id=eq.${user_id}`);
...
export async function addClient(newCollaborator) {
  try {
    const response = await supabase.post('/rest/v1/todo_postorota', newCollaborator);
...
export async function deleteClient(user_id) {
  try {
    await supabase.delete(`/rest/v1/todo_postorota?id=eq.${user_id}`);
  } catch (error) {
...
export async function editStructure(itemId, data) {   
  ...
  try {
    await supabase.put(`/rest/v1/todo_postorota?id=eq.${itemId}`, data);
  } catch (error) {
