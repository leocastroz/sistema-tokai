...
export async function getProfessor(user_id) {
  try {
    const response = await supabase.get(
      `/rest/v1/produtos_tokai?select=*&user_id=eq.${user_id}`
    );
    return response.data;
  } catch (error) {
    throw new Error(`Erro ao obter os produtos_tokai: ${error.message}`);
  }
}
...
export async function getProdutoById(user_id, id) {
  try {
    const response = await supabase.get(
      `/rest/v1/produtos_tokai?select=*&user_id=eq.${user_id}&id=eq.${id}`
    );
    return response.data;
  } catch (error) {
    throw new Error(`Erro ao obter os produtos_tokai: ${error.message}`);
...
export async function deleteProfessor(professorId) {
  try {
    await supabase.delete(`/rest/v1/produtos_tokai?id=eq.${professorId}`);
  } catch (error) {
...
export async function editProfessor(professorId, data) {
  try {
    await supabase.put(`/rest/v1/produtos_tokai?id=eq.${professorId}`, data);
  } catch (error) {
