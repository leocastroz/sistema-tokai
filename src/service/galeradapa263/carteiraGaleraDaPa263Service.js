...
export async function getCarteira(user_id) {
  try {
    const response = await supabase.get(`/rest/v1/carteira_galeradapa263?select=*&user_id=eq.${user_id}`);
...
export async function addCarteira(newCarteira) {
  try {
    const response = await supabase.post('/rest/v1/carteira_galeradapa263', newCarteira);
    return response.data;
  } catch (error) {
...
export async function deleteCarteira(user_id) {
  try {
    const response = await supabase.delete(`/rest/v1/carteira_galeradapa263?user_id=eq.${user_id}`);
