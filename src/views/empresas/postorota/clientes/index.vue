<template>
  <div>
    <div class="flex justify-between items-center">
      <p class="text-xl w-full font-extrabold text-white animate__animated animate__fadeIn">POSTO ROTA - CLIENTES</p>
...
import { deleteCliente, getCliente, postClient, putClient } from '@/service/postorota/clientesPostoRotaService.js';
...
const toggleModalVenda = (professorObj) => {
  ...
  if (professorObj) {
    ...
    newVenda.value.email = professorObj.email;
    newVenda.value.cpf = professorObj.cpf;
    newVenda.value.whatsapp = professorObj.whatsapp;
...
async function addNewProduto() {
  ...
  try {
    ...
    const newEmployeeData = {
      ...
      cpf: newVenda.value.cpf,
      email: newVenda.value.email,
    ...
    // console.log('VALOR RESPONSE 33333 ->', response3);
    ...
    fetchMeusProdutos();
    fetchVendasRealizadas();
    newVenda.value = { cliente_id: "", produto: "", nome: "", whatsapp: "", cpf: "", email: "", descricao: "", marca: "", totalModelo: "", campoDesconto: "", valor: "", tipo: "", fileName: "", selectedImageFile: null };
    selectedImageFile.value = null;
    isModalRegisterVisible.value = false;
    // notification.resolve('Dados adicionados com sucesso!');
