      ...
      <table class="w-full">
        <thead class="border-solid border-b-[1px] border-[#363636] text-gray-500">
          <tr class="text-xs">
            ...
            <th class="text-left w-72 px-3">NOME CLIENTE</th>
            <th class="text-left w-64 px-3 ">CPF CLIENTE</th>
            <th class="text-left w-64 px-3 ">WHATSAPP CLIENTE</th>
            <th class="text-left w-64 px-3 ">E-MAIL CLIENTE</th>
            <th class="text-center w-64 px-3 ">QUANT. VENDIDA</th>
            <!-- <th class="w-80">AÇÕES</th> -->
          </tr>
        </thead>
        <tbody v-if="filteredProfessors.length > 0" class="text-[13px]">
          <tr v-for="professor in paginatedActiveProfessors" :key="professor.id"
            ...
            <td class="flex justify-center w-20 animate__animated animate__fadeIn py-2">
              <!-- <pre>{{ professor }}</pre> -->
              <!-- URL PARA SEM IMAGEM ABAIXO -->
              <!-- https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRscuhvdtB4YZO11JgFkbpLCQSgFMydCLHZWQ&s -->
            ...
            <td class="text-left px-3 w-64 animate__animated animate__fadeIn">{{ professor.cpf }}</td>
            <td class="text-left px-3 w-64 animate__animated animate__fadeIn">{{ professor.whatsapp }}</td>
            <!-- <td class="text-left px-3 w-64 animate__animated animate__fadeIn">{{ professor.totalModelo }}</td> -->
            <td class="text-left px-3 w-64 animate__animated animate__fadeIn">{{ professor.email }}</td>
            <td class="text-center animate__animated animate__fadeIn text-white"><span class=" bg-gradient-to-r from-blue-600 via-blue-500 to-blue-400 rounded-xl py-1 px-2">{{ professor.quantidade }}</span></td>
            ...
            <!-- <td class="animate__animated animate__fadeIn w-80 text-center">
              <i @click="$emit('delete', professor)" class="fa-solid fa-trash text-[13px] text-white bg-red-500 items-center p-2 mx-1 rounded drop-shadow-md cursor-pointer hover:bg-red-700" v-tooltip="'Deletar'"></i>
              ...
              <i class="fa-solid fa-lock text-[13px] text-white items-center p-2 mx-1 rounded drop-shadow-md cursor-pointer hover:bg-gray-600" @click="$emit('toggleStatus', professor.id)" :class="`material-symbols-outlined ${professor.status ? 'bg-gray-500' : 'bg-gray-500'}`" v-tooltip="'desativar'"></i>
              <i @click="$emit('venda', professor)" class="fa-solid fa-arrow-trend-up text-[13px] text-green-400 items-center p-2 mx-1 rounded drop-shadow-md cursor-pointer hover:bg-gray-600" v-tooltip="'Lançar vendas'"></i>
            </td> -->
          </tr>
        </tbody>
      </table>
      <div v-if="activeProfessors.length === 0" class="p-3 text-center text-gray-500">
      ...
      <table class="w-full">
        <thead class=" text-gray-500">
          <tr class="text-xs">
            ...
            <th class="text-left w-64 px-3 ">CPF</th>
            <th class="text-left w-64 px-3 ">NÚMERO TEL.</th>
            <th class="text-left w-64 px-3 ">E-MAIL</th>
            <!-- <th class="text-left w-64 px-3 ">MARCA</th> -->
            <th class="text-left w-64 px-3 ">TOT. MODELO</th>
            <!-- <th class="text-left w-64 px-3 ">DESCONTO</th> -->
            <th class="text-left w-64 px-3 ">ENTRADA</th>
        ...
        <tbody v-if="inactiveProfessors.length > 0" class="text-[13px]">
          <tr v-for="professor in paginatedInativeProfessors" :key="professor.id"
            ...
            <!-- <td class="text-left px-3 w-64 animate__animated animate__fadeIn">{{ professor.totalModelo }}</td> -->
            <td class="text-left px-3 w-64 animate__animated animate__fadeIn">{{ professor.email }}</td>
            <td class="text-left px-3 w-64 animate__animated animate__fadeIn">{{ professor.entrada }}</td>
            <!-- <td class="text-left px-3 w-64 animate__animated animate__fadeIn">R$ {{ professor.valor % 1 === 0 ? professor.valor + ',00' : professor.valor.toLocaleString('pt-BR', { minimumFractionDigits: 2 }) }}</td> -->
...
const activeProfessors = computed(() => {
  return props.tgringaRoupas.filter(professor => professor.status && professor.nome.toLowerCase().includes(searchQuery.value.toLowerCase()));
});
...
function filterProfessors() {
  ...
  const filtered = props.tgringaRoupas.filter((professor) => {
    return professor.nome.toLowerCase().includes(query);
  });
