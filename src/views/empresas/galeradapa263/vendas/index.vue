<template>
  <div>
    <div class="flex justify-between items-center">
      <p class="text-xl w-full font-extrabold text-white animate__animated animate__fadeIn">GALERA DA PA - VENDAS</p>
    ...
    <register v-show="isModalRegisterVisible" :professor="newCollaborator" :marcas="marcas" @register="addNewEmployee" @push-marca="pushedmarca" @cancel="toggleModalRegister" @uploadImage="selectedImageFile = $event" />
...
import { deleteVenda, getVendas, postVendas, putVendas } from '@/service/galeradapa263/vendasGaleraDaPa263Service.js';
...
async function toggleStatus(professorId) {
  ...
  if (professorToToggle) {
    ...
    try {
      const response = await putVendas(professorToToggle.id, professorToToggle);
      fetchProfessor();
...
async function updateEmployee() {
  ...
  try {
    const response = await putVendas(editedProfessor.value.id, editedProfessor.value);
...
async function fetchProfessor() {
  try {
    ...
    // console.log('PEGANDO TODAS AS VENDAS', tgringaRoupas.value);
  } catch (error) {
    console.log(error)
...
async function addNewEmployee() {
  ...
  try {
    ...
    newCollaborator.value = { image_one: "", nome: "", descricao: "", marca: "", totalModelo: "", campoDesconto: "", valor: "", tipo: "", fileName: "", selectedImageFile: null };
    selectedImageFile.value = null;
