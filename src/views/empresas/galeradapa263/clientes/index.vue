<template>
  <div>
    ...
    <venda
      v-show="isModalVendaVisible"
      :professor="vendaProfessor"
      :addproduto="newVenda"
      :listvendas="listaVendas"
      :produtos="produtosCliente"
      @update="updateVendas"
      @cancel="toggleModalVenda"
      @register="addNewProduto"
      @uploadImage="handleImageUpload"
      @deleteImage="handleImageDelete"
      style="position: fixed; left: 0; top: 0; width: 100vw; height: 100vh"
    />
...
import {
  ...
  postClient,
  putClient,
} from "@/service/galeradapa263/clientesGaleraDaPa263Service.js";
import {
  getProfessor,
  editProfessor,
  getProdutoById,
...
import {
  ...
  postVendas,
  putVendas,
} from "@/service/galeradapa263/vendasGaleraDaPa263Service.js";
  ...
  if (professorObj) {
    ...
    if (professorObj.id) {
      const professorToEdit = tgringaRoupas.value.find(
        (professor) => professor.id === professorObj.id
      );
    ...
    newVenda.value.fileName = professorObj.fileName;
  } else {
    vendaProfessor.value = {
      produto: "",
      nome: "",
...
async function updateEmployee() {
  if (
    !editedProfessor.value.nome ||
    !editedProfessor.value.formacao ||
    !editedProfessor.value.whatsapp ||
    !editedProfessor.value.descricao ||
    !editedProfessor.value.cpf ||
    !editedProfessor.value.email ||
    !editedProfessor.value.entrada ||
    !editedProfessor.value.data_nascimento ||
    !editedProfessor.value.tipo
  ) {
...
async function updateVendas() {
  if (
    !vendaProfessor.value.nome ||
    !vendaProfessor.value.whatsapp ||
    !vendaProfessor.value.descricao ||
    !vendaProfessor.value.marca ||
    !vendaProfessor.value.totalModelo ||
    !vendaProfessor.value.campoDesconto ||
    !vendaProfessor.value.tipo
  ) {
    push.error("Todos os campos são obrigatório!");
    console.error("Error: Todos os campos são obrigatórios");
    return;
  ...
  try {
    const response = await putVendas(
      vendaProfessor.value.id,
      vendaProfessor.value
    );
...
async function addNewProduto() {
  ...
  try {
    ...
    let totalAtualProdutosId =
      response3[0].totalModelo - newVenda.value.quantidade;
    const response2 = await editProfessor(newVenda.value.produto, {
      id: newVenda.value.produto,
      user_id: userIn,
      totalModelo: totalAtualProdutosId,
    });
