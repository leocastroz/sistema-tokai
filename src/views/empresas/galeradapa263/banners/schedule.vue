    ...
    <div v-if="selectedStatus === 'true'">
      ...
      <table class="w-full">
        <thead class="border-solid border-b-[1px] border-[#363636] text-gray-500">
          <tr class="text-xs">
            <th class="text-left w-24 p-2">IMG</th>
            <th class="text-left w-72 px-3">NOME</th>
            <th class="text-left w-64 px-3 ">DESCRIÇÃO</th>
            <th class="text-left w-64 px-3 ">CATEGORIA</th>
        ...
        <tbody v-if="filteredProfessors.length > 0" class="text-[13px]">
          <tr v-for="professor in paginatedActiveProfessors" :key="professor.id"
            ...
            <td class="flex justify-center w-24 animate__animated animate__fadeIn py-2">
              <img :src="professorImageURL(professor)" @error="setDefaultImage" class="w-20 rounded" />
            </td>
            ...
            <td class="animate__animated animate__fadeIn w-56 text-center">
              ...
              <i class="fa-solid fa-pen-to-square text-[13px] text-white bg-blue-500 items-center p-2 mx-1 rounded drop-shadow-md cursor-pointer hover:bg-blue-700" @click="$emit('edit', professor.id)" v-tooltip="'editar'"></i>
        ...
        <tbody v-if="getDataTable == true">
          <tr>
            <td colspan="7" class="p-3 text-center text-gray-500">
              <p class="pt-4 text-gray-400">Buscando banners...</p>
            </td>
          </tr>
        </tbody>
      </table>
      ...
      <table class="w-full">
        ...
        <tbody v-if="inactiveProfessors.length > 0" class="text-[13px]">
          <tr v-for="professor in paginatedInativeProfessors" :key="professor.id"
            ...
            <td class="text-left px-3 w-64 animate__animated animate__fadeIn">{{ professor.categoria }}</td>
            <td class="animate__animated animate__fadeIn w-56 text-center">
              <span @click="$emit('toggleStatus', professor.id)" :class="`material-symbols-outlined text-[18px] text-white px-1 items-center p-1 mx-1 rounded drop-shadow-md cursor-pointer hover:bg-gray-500 ${professor.status ? 'bg-green-300' : 'bg-gray-300'}`" v-tooltip="'ativar'">key</span>
            </td>
          </tr>
        </tbody>
      </table>
