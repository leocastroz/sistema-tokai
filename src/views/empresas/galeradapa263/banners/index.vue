<template>
  <div>
    <div class="flex justify-between items-center">
      <p class="text-xl w-full font-extrabold text-white animate__animated animate__fadeIn">GALERA DA PA - BANNERS</p>
    ...
    <register v-show="isModalRegisterVisible" :professor="newCollaborator" :marcas="marcas" @register="addNewEmployee" @push-marca="pushedmarca" @cancel="toggleModalRegister" @uploadImage="selectedImageFile = $event" style="position: fixed;left: 0;top: 0;width: 100vw;height: 100vh;" />  
...
import { deleteBanner, getBanner, addBanner, editBanner } from '@/service/galeradapa263/bannersGaleraDaPa263Service.js';
...
async function toggleStatus(professorId) {
  ...
  if (professorToToggle) {
    ...
    try {
      const response = await editBanner(professorToToggle.id, professorToToggle);
      fetchProfessor();
...
let editedProfessor = ref({ nome: "", descricao: "", categoria: "", fileName: "" });
...
async function updateEmployee() {
  ...
  try {
    const response = await editBanner(editedProfessor.value.id, editedProfessor.value);
...
async function deleteImage(professor, fileName) {
  try {
    let user_id = await getUserId()
    const { data, error } = await supabase.storage.from('banners' + '/' + user_id + '/' + professor).remove([fileName && professor]);
    if (error) {
      throw error;
    }
...
async function addNewEmployee() {
  ...
  try {
    ...
    newCollaborator.value = { nome: "", descricao: "", categoria: "", fileName: "", selectedImageFile: null };
    selectedImageFile.value = null;
